...
If there's anything not quite right with your implementation, `check50` won't provide you with any hints this time. Instead, you might want to try creating an instance of `Queue` yourself to see if everything works as expected. Especially make sure that `dequeue()` throws an `AssertionError` if the queue is empty. 
